...
- **Manual regeneration during automatic generation can race** → Use digest status and per-user/date/window lookup to avoid duplicate active jobs. Regeneration is blocked or returns the active state while any same-window job is pending/running; after terminal state, an explicit regeneration may overwrite the selected digest.
